...
if [ -d "$1" ] ; then
    ...
    if [ -d "$2" ] ; then
        UP="\.\.\/"
        for i in "$2/de/phoneScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
        for i in "$2/en/phoneScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
        for i in "$2/de/sevenInchScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
        for i in "$2/en/sevenInchScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
        for i in "$2/de/tenInchScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
        for i in "$2/en/tenInchScreenshots/"* ; do [ -f "$i" ] && ln -s "$(echo $i | sed "s/$2/$UP$UP$UP$2/")" "$(echo $i | sed "s/$2/$1/")" ;  done;
    fi
